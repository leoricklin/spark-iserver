...
|-- id: string (nullable = true)
    ...
    val t42 = sqlContext.sql("select AgentData.NowTime, id" +
    ...
    t42.collect: Array[Row]
     = Array([1441872159969,31208,0.098273,ArrayBuffer([phy_usage,61.927822], [cache_usage,0.043043], [memory_load,40.852562]),ArrayBuffer([outbound,3], [inbound,303], [packet_send_error,0], [packet_recv_error,0])])
  ...
  def saveBasicRecords(parsedLogs: RDD[BasicRecord], path:String) = {
    parsedLogs.coalesce(64).saveAsParquetFile(path)
    /*
...
-rw-rw----+  3 leoricklin hive    1228008 2015-09-14 16:28 hdfs://nameservice1/hive/tlbd_upload/iserver/parquet/complex.20150914/part-r-10.parquet
  ...
  def readBasicRecords(sqlContext:org.apache.spark.sql.SQLContext, path:String) = {
    val basicRecords = sqlContext.parquetFile(path)
    basicRecords
    /*
...
,"hdfs:///hive/tlbd_upload/iserver/parquet/basic.20150915"
,"hdfs:///hive/tlbd_upload/iserver/parquet/complex.20150915")
...
278
    ...
    try {
      ...
      val basicRecords = readBasicRecords(sqlContext, basicoutpath)
    } catch {
